...
named criterion → carry a re-checkable receipt; no learned authority in the accept
path.** Forward it is a projection (conserve the criterion, discard the rest). Backward
it is reconstruction up to the discarded null space. Driven as a loop it compounds and
springloads; gated one-way it ratchets; and every self-authored shortcut leaks exactly
...
- **Amplifiers**: amortization (permission vs accountability, `authorization_cost`),
  verified lift (M7), inversion acceleration (`inversion_flywheel`), fluid springload
  (`fluid_router`).
...
of internally-grounded indicators that survive manipulation, confound removal, and
distribution shift."** That is exactly `crucible` + `adversarial_corpus` +
`externalization_ablation` + the self-authored-criterion ablation. Its proposed benchmarks
map onto real modules:
...
target. The superproject is agnostic and precautionary, exactly as the report concludes.

## What is real vs aspirational

- **Real, committed, tested**: the engine (M0–M7 + trained model), the five-organ spine over
  live flagships, the mechanism catalog, the honest boundaries.
- **Pending**: the self-authored-criterion ablation (earns or shrinks the capability claim).
- **Aspirational, labeled as such**: none of the cross-domain modules is a new law; each is
  reach demonstrated across a substrate, honestly bounded. No claim of phenomenal
  consciousness, no unbounded launch variable, no discovered unification.
